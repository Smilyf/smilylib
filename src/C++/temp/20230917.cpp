...
public:
    ...
    smilyIoRing()
    {
        ...
        userDataDequesize = t.MaxSubmissionQueueSize;
    ...
    void smilyBuildIoRingRegisterBuffers(UINT32 size)
    {
        ...
        // int registerBuffersDequesize=registerBuffersDeque.size();
        for (UINT32 i = 0; i < count; i++)
        {
            setregisterBuffers(bufferInfo.data()[i].Address);
            // registerBuffersDeque.push_back();
        }
        smilySubmitIoRing();
    }
